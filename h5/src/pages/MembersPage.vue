...
async function refresh() {
  loading.value = true
  try {
    const res = await apiFetch<any>('/api/v1/app/members?page=1&pageSize=50')
...
<template>
  <AppHeader title="会员单位" :showBack="true" />
  <main class="app-container page page-pad">
    <section class="glass search">
      <input v-model="keyword" class="input" placeholder="搜索单位名称" />

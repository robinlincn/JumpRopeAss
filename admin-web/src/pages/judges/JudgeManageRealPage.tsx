...
export function JudgeManagePage() {
  ...
  const loadData = async (p = page, s = size, kw = keyword) => {
    ...
    try {
      ...
      const res = await apiFetch<any>(`/api/v1/admin/judges?${qs.toString()}`)
      if (res.code !== 0) {
  ...
  const columns: ColumnsType<RowItem> = [
    ...
    { title: '等级', dataIndex: 'judgeLevel', width: 120 },
          ...
          <Button
            ...
            onClick={() => {
              ...
              form.setFieldsValue({
                ...
                orgId: r.orgId,
                judgeLevel: r.judgeLevel ?? undefined,
                status: r.status,
              })
              setDrawerOpen(true)
            }}
          ...
          <Button
            ...
            onClick={() => {
              Modal.confirm({
                ...
                cancelText: '取消',
                onOk: async () => {
                  const resp = await apiFetch(`/api/v1/admin/judges/${r.id}`, { method: 'DELETE' })
                  if (resp.code !== 0) {
            ...
            <Button
              ...
              onClick={async () => {
                ...
                const dto = {
                  ...
                  judgeLevel: v.judgeLevel,
                  status: v.status,
                }
                const resp = editing
                  ? await apiFetch(`/api/v1/admin/judges/${editing.id}`, { method: 'PUT', body: JSON.stringify(dto) })
                  : await apiFetch('/api/v1/admin/judges', { method: 'POST', body: JSON.stringify(dto) })
                if (resp.code !== 0) {
